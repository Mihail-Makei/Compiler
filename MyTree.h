...
node_t* NodeCreate(int data, int type)
{
...
void NodeFill(node_t* node, int type, int data)
{
    node->data.type = type;
    node->data.data = data;
}
...
void TreeGraph(FILE* fout, node_t* node)
{
    ...
    if (node->parent != nullptr)
    {
        ...
        fprintf(fout, "\"");
        fprintf(fout, "%p\tTYPE\t%d\tDATA\t%d", node->parent, node->parent->data.type, node->parent->data.data);
        fprintf(fout, "\" -> \"");
        fprintf(fout, "%p\tTYPE\t%d\tDATA\t%d", node, node->data.type, node->data.data);
        fprintf(fout, "\";\n");
    }
